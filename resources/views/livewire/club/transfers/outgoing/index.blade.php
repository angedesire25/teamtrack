<div>
    <div class="flex items-center gap-3 mb-6">
        ...
        <div class="flex-1">
            <h1 class="text-2xl font-extrabold text-gray-900">Transferts sortants</h1>
            <p class="text-gray-500 text-sm mt-0.5">Joueurs mis sur liste de transfert</p>
        </div>
    ...
    <div class="bg-amber-50 border border-amber-200 rounded-xl px-4 py-3 mb-4 text-sm text-amber-700 flex items-center gap-2">
        ...
        Hors fenêtre de transfert — les opérations peuvent être préparées mais ne seront effectives qu'en période de mercato.
    </div>
    @endif
        ...
        <table class="w-full text-sm">
            <thead>
                <tr class="border-b border-gray-100 bg-gray-50/50">
                    ...
                    <th class="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wide hidden md:table-cell">Club acheteur</th>
                    <th class="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wide">Statut</th>
                    <th class="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wide hidden lg:table-cell">Prix demandé</th>
            ...
            <tbody class="divide-y divide-gray-50">
                ...
                <tr class="hover:bg-gray-50">
                    <td class="px-5 py-3.5">
                        ...
                        <p class="text-xs text-gray-400">{{ $t->player?->position ?? '' }}</p>
                    </td>
                    ...
                    <td class="px-4 py-3.5 hidden lg:table-cell font-semibold text-gray-700">
                        {{ $t->asking_price ? number_format($t->asking_price,0,'.',' ').' F' : '—' }}
                    </td>
                ...
                @empty
                <tr><td colspan="7" class="px-5 py-12 text-center text-gray-400">Aucun transfert sortant</td></tr>
                @endforelse
            </tbody>
        </table>
    ...
    <div class="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md" @click.stop>
            <div class="px-6 py-5 border-b border-gray-100 flex items-center justify-between">
                <h3 class="font-bold text-gray-900">Nouveau transfert sortant</h3>
            ...
            <form wire:submit="save" class="px-6 py-5 space-y-4">
                <div>
                    ...
                    <select wire:model="playerId" class="w-full px-3.5 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-[#1E3A5F]/20 focus:border-[#1E3A5F] bg-white">
                        <option value="">Sélectionner un joueur…</option>
                        @foreach($players as $p)
                        <option value="{{ $p->id }}">{{ $p->fullName() }}{{ $p->position ? ' · '.$p->position : '' }}</option>
                        @endforeach
                    </select>
                    @error('playerId') <p class="mt-1 text-xs text-red-500">{{ $message }}</p> @enderror
                </div>
                <div>
                ...
                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-1.5">Prix demandé (F CFA)</label>
                    <input wire:model="asking_price" type="number" min="0" step="1000" placeholder="Ex : 50 000 000"
                ...
                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-1.5">Club intéressé (optionnel)</label>
                    <input wire:model="counterpart_club" type="text" placeholder="Nom du club acheteur"
                           class="w-full px-3.5 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-[#1E3A5F]/20 focus:border-[#1E3A5F]">
                </div>
                <div>
